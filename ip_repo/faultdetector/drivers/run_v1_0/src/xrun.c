...
FAULTDETECTOR_isReadyForNextControl(XRun *InstancePtr, u8 taskId, FAULTDETECTOR_OutcomeStr* dest) {
	return (*((char*) (InstancePtr->Control_BaseAddress+XRUN_CONTROL_ADDR_COPYINPUTAOV_DATA)))==0;
}

FAULTDETECTOR_processNextControl() {
...
FAULTDETECTOR_OutcomeStr FAULTDETECTOR_getLastError(XRun *InstancePtr, u8 taskId, FAULTDETECTOR_OutcomeStr* dest) {
	memcpy((void*) dest, (void*) (InstancePtr->Control_BaseAddress+XRUN_CONTROL_ADDR_OUTCOMEINRAM_BASE+XRUN_CONTROL_WIDTH_OUTCOMEINRAM*taskId), sizeof(FAULTDETECTOR_OutcomeStr));
}
...
char FAULTDETECTOR_isFault(XRun *InstancePtr, u8 taskId) {
	return *((char*) (InstancePtr->Control_BaseAddress+XRUN_CONTROL_ADDR_ERRORINTASK_BASE+taskId*XRUN_CONTROL_WIDTH_ERRORINTASK));
}

void FAULTDETECTOR_resetFault(XRun *InstancePtr, u8 taskId) {
	*((char*) (InstancePtr->Control_BaseAddress+XRUN_CONTROL_ADDR_ERRORINTASK_BASE+taskId*XRUN_CONTROL_WIDTH_ERRORINTASK))=0;
}
